...
import argparse
from snipe.patholib import pathoLib
from snipe.pathomap import PathoMapA
from snipe.pathoid import PathoID
from snipe.pathorec import PathoRec
...
parser_a = subparsers.add_parser('LIB', help='Snipe taxon level reference genome Library creation Module')
...
parser_a.add_argument('-db', action='store', dest='lib_db', required=False, default='pathodb', 
	help='mysql snipe database name (default: pathodb)')
...
parser_b.add_argument('-targetAlignParams', action='store', 
	...
	help='Target Mapping Bowtie2 Parameters (Default: Snipe chosen best parameters)')
...
#create the parser for the "RECTIFY" command
parser_d = subparsers.add_parser('REC', help='Snipe REC Module')
#parser_d.add_argument('-i', action='store', dest='lib_reference', required=True, 
#help='Specify reference genome (Folder contains reference sequence)')
parser_d.add_argument('-ssrRef', action='store', dest='ssr_reference', required=True, 
help='the directory of the species specific regions')
#parser_d.add_argument('-U', default='', action='store', dest='rec_inputread', required=False, 
#	help='Input Read Fastq File (Unpaired/Single-end)')
parser_d.add_argument('-1', default='', action='store', dest='rec_inputread1', required=False, 
	help='Input Read Fastq File (Pair 1)')
parser_d.add_argument('-2', default='', action='store', dest='rec_inputread2', required=False, 
help='Input Read Fastq File (Pair 2)')
parser_d.add_argument('-idReport', action='store', dest='id_rep_file', required=True,
	help='Report file generated by ID module')
parser_d.add_argument('-dictTarget', action='store', dest='dict_target', required=True,
	help='Target strain information')
parser_d.add_argument('-dictTemplate', action='store', dest='dict_template', required=True,
	help='the dict which contain the strain to name mapping')
parser_d.add_argument('-outDir',action='store',default='.',dest='rec_outDir',required=False,
help='Output Directory (Default=.(current directory))')
...
parser_e = subparsers.add_parser('REP', help='Snipe Report Module')
parser_e.add_argument('-samtoolsHome', default=None, action='store', 
	dest='rep_samtoolshome', required=False, 
	help='Full Path to samtools binary directory (Default: Uses samtools in system path)')
parser_e.add_argument('-dbhost', action='store', dest='rep_dbhost', required=False, 
	default='localhost', help='specify hostname running mysql if you want to use mysql '
	'instead of hash method in mapping gi to taxonomy id')
parser_e.add_argument('-dbport', action='store', dest='rep_dbport', required=False, 
	default=3306, type=int, help='provide mysql server port if different from default (3306)')
parser_e.add_argument('-dbuser', action='store', dest='rep_dbuser', required=False, default='X', 
	help='user name to access mysql')
parser_e.add_argument('-dbpasswd', action='store', dest='rep_dbpasswd', required=False, default='X', 
	help='provide password associate with user')
parser_e.add_argument('-db', action='store', dest='rep_db', required=False, default='pathodb', 
	help='mysql snipe database name (default: pathodb)')
parser_e.add_argument('-outDir', action='store', default='.', dest='rep_outdir',
					help='Output Directory')
parser_e.add_argument('--contig', action='store_true', default=False, dest='rep_contig_flag',
	help='Generate Contig Information (Needs samtools package installed)')
parser_e.add_argument('-samfile', action='store', dest='rep_ali_file', required=True,
					help='SAM Alignment file path')
parser_e.add_argument('--noDisplayCutoff', action='store_true', default=False, 
	dest='rep_nocutoff', help='Do not cutoff display of genomes, even if it is insignificant')

parser_f = subparsers.add_parser('QC', help="PathoQC: Quality control program for high throughput sequencing reads")
parser_f.add_argument('-1', action='store', dest='qc_read1', required=True, default='', help='1st pair of read in PER or SER')
parser_f.add_argument('-2', action='store', dest='qc_read2', required=False, default='', help='2nd pair of read in PER')
parser_f.add_argument('-r', action='store', dest='qc_readL', required=False, default=0, type = int, help='let us know a mean read length (0:ignore. [1]:I want to know the range of read length after trimming, i:user_specified_mean_read_length)')
parser_f.add_argument('-t', action='store', dest='qc_phred_offset', required=False, default=33, type = int, help='specify a phred offset used in encoding base quality(0:not sure?, [33]:phred+33, 64:phred+64)')
parser_f.add_argument('-o', action='store', dest='qc_outdir', required=False, default='', help='specify output directory in full path')
parser_f.add_argument('-s', action='store', dest='qc_sequencer', required=False, default='Illumina', help='tell us which sequencer generates the reads ([Illumina], PacBio, Roche454, IonTorrent)')
parser_f.add_argument('-m', action='store', dest='qc_len_cutoff', required=False, type=int, default=35, help='specify the min read length cutoff[35]')
parser_f.add_argument('-a', action='store', dest='qc_adaptor', required=False, default='N', help='specify an adaptor (Y:have pathoQC detect it, [N]:ignore, ACGT:adaptor)')
parser_f.add_argument('-a2', action='store', dest='qc_adaptor2', required=False, default='N', help='specify a second adaptor if it is different from the first one (Y:have pathoQC detect it, [N]:ignore, ACGT:adaptor)')
#parser_f.add_argument('-k', action='store', dest='qc_primer', required=False, default='N', help='specify a primer')
parser_f.add_argument('-q', action='store', dest='qc_qual_cutoff', required=False, type=int, default=0, help='specify a cutoff of base quality value to trim at the end of reads([0]:ignore, 1:let pathoQC take care of it, i:user_specified_cutoff_value)')
parser_f.add_argument('-R', action='store', dest='qc_lower454', required=False, type=int, default=0, help='set to 1 if you want to mask all lower case bases that may correspond to sequence tag or adaptor in Roche454 or IonTorrent')
parser_f.add_argument('-e', action='store', dest='qc_coff_entropy', required=False, type=int, default=30, help='specify a cutoff of entropy of low sequence complexity to filter out[0..100],default:30, set 0 to disable')
parser_f.add_argument('-d', action='store', dest='qc_derep', required=False, type=int, default=1, help='filtering duplicates: [1] (exact duplicate), 2 (5\' duplicate), 3 (3\' duplicate), 4 (reverse complement exact duplicate), 5 (reverse complement 5\'/3\' duplicate)');
parser_f.add_argument('-g', action='store', dest='qc_add_valid_single', required=False, type=int, default=0, help='Set to 1 if you want to add a valid single read into a reduced valid PER set. Note that this option works only with PER')
parser_f.add_argument('--no_prinseq', action='store_const', dest='qc_on_prinseq', required=False, const=False, default=True, help='to force to skip prinseq module')

parser_f.add_argument('-p', action='store', dest='qc_num_threads', required=False, type=int, default=1, help='specify a total number of cpus to use[1]')
parser_f.add_argument('--debug', action='store_const', dest='qc_debugF', required=False, const=True, default=False, help='working on debug mode')
parser_f.add_argument('--version', action='version', version='PathoQC 1.0')
